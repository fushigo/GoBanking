...
namespace GoBanking {
	...
	System::Void CustomerData::ShowConfirmationPopup() {
		...
		currentPopup->SetActionButton1("Konfirmasi", gcnew EventHandler(this, &CustomerData::OnConfirmTransfer));
		currentPopup->SetActionButton2("Batal", gcnew EventHandler(this, &CustomerData::OnClose));
		currentPopup->ShowPopup();
	}

	System::Void CustomerData::OnConfirmTransfer(System::Object^ sender, System::EventArgs^ e) {
	...
	System::Void CustomerData::ShowResultPopup(bool isSuccess) {
		currentPopup = gcnew PopupForm();
		if (isSuccess) {
			currentPopup->SetMessage("Berhasil menghapus data!");
			currentPopup->SetActionButton1("OK", gcnew EventHandler(this, &CustomerData::OnResultConfirmed));
		}
		else {
			currentPopup->SetMessage("Gagal menghapus data!");
			currentPopup->SetActionButton1("Coba Lagi", gcnew EventHandler(this, &CustomerData::OnRetryTransfer));
		}
	...
	System::Void CustomerData::OnRetryTransfer(System::Object^ sender, System::EventArgs^ e) {
		if (currentPopup != nullptr) {
			currentPopup->ClosePopup();
			ShowConfirmationPopup();
		}
